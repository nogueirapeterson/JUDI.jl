...
# Setup operators
opt = Options(optimal_checkpointing=false, f0=f0)  # ~40 GB of memory per source w/o checkpointing
...
fhistory_dir = joinpath(output_dir, "fhistory_viscoacoustic_sgd_sls_2nd.bin")
write(fhistory_dir, htol.(transpose(fval)));

x_m = x
x_m_reshape = reshape(x_m, model0.n)
imagem_dir = joinpath(output_dir, "xm_final_sls_2nd.bin")
