...
fhistory_dir = joinpath(output_dir, "fhistory_viscoacoustic_multi_parameter_sgd.bin")
write(fhistory_dir, htol.(transpose(fval)));

x_m = x[1]
x_m_reshape = reshape(x_m, model0.n)
imagem_dir = joinpath(output_dir, "xm_final_test.bin")
write(imagem_dir, htol.(transpose(x_m_reshape)));

x_tau = x[2]
x_tau_reshape = reshape(x_tau, model0.n)
imagem_dir = joinpath(output_dir, "xtau_final_test.bin")
